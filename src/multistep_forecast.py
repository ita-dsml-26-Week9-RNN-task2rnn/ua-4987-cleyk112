...
def make_windows(series: np.ndarray, window: int, horizon: int = 1) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for t in range(window, len(series) - horizon + 1):
        X_list.append(series[t-window:t])
        y_list.append(series[t:t+horizon])

    # convert to numpy and reshape
    X = np.array(X_list)  # shape (N, window)
...
def recursive_rollout_one_step(
    ...
    3) shift window by 1
    """
    window = init_window.copy().tolist()
    forecast = []
    
    for _ in range(horizon):
        x = np.array(window).reshape(1, -1, 1)
...
def recursive_rollout_k_step_stride_k(
    ...
    for _ in range(horizon // k):
        x = np.array(window).reshape(1, -1, 1)
        preds = model.predict(x, verbose=0)[0]  # shape (k,)
        forecast.extend(preds)
        window = window[k:] + preds.tolist()
...
def recursive_rollout_k_step_stride_1(
    ...
    for _ in range(horizon):
        ...
        preds = model.predict(x, verbose=0)[0]  # shape (k,)
        first_pred = preds[0]
        forecast.append(first_pred)
        window.append(first_pred)
        window.pop(0)
